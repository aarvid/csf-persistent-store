...
(defconstant +ascii-linefeed+        10
  "The integer that represents the ascii LF character.")
(defconstant +ascii-carriage-return+ 13
  "The integer that represents the ascii CR character.")
(defconstant +ascii-zero+            48
  "The integer that represents the ascii digit 0.")
...
(define-constant +character-map/ascii->lisp+
...
(define-constant +ascii-code-set/alpha+
  (load-time-value (predicate->ascii-code-set #'alpha-char-p))
  :test #'equalp)

(define-constant +ascii-code-set/alphanumeric+
  (load-time-value (predicate->ascii-code-set #'alphanumericp))
  :test #'equalp)

(define-constant +ascii-code-set/decimal-digit+
  (load-time-value
   (predicate->ascii-code-set
    (lambda (char) (digit-char-p char 10.))))
  :test #'equalp)

(define-constant +ascii-code-set/hex-digit+
  (load-time-value
   (predicate->ascii-code-set
