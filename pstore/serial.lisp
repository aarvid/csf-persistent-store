...
||#
  ...
  ;; $$$ gave error in sbcl
  ;; so wrapped object with function package-name to get a string
  ;; seems to match with deserialize.
  (:method ((object package) stream symbol-table)
    (write-byte serialization-code/package stream)
    (write-fixnum (symbol-table/intern-string symbol-table (package-name object))
                  stream))
